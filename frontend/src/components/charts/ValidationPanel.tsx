  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <h4 className="text-sm font-semibold">{t.mcTitle}</h4>
        <Badge value={sig ? t.mcSignificant : t.mcNotSignificant} good={sig} />
      </div>
      <p className="text-xs text-muted-foreground">
        {t.mcDesc.replace("{n}", mc.n_simulations.toLocaleString())}
      </p>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-1 rounded-xl border border-border/60 bg-muted/20 p-3">
        <Stat label={t.mcActualSharpe} value={mc.actual_sharpe.toFixed(2)} />
        <Stat label={t.mcPValue} value={mc.p_value_sharpe.toFixed(4)} sub={sig ? "< 0.05" : ">= 0.05"} />
        <Stat label={t.mcSimulatedMean} value={mc.simulated_sharpe_mean.toFixed(2)} sub={`std ${mc.simulated_sharpe_std.toFixed(2)}`} />
        <Stat label={t.mcSimulatedRange} value={`[${mc.simulated_sharpe_p5.toFixed(2)}, ${mc.simulated_sharpe_p95.toFixed(2)}]`} />
      </div>
...
function BootstrapSection({ bs }: { bs: NonNullable<ValidationData["bootstrap"]> }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <h4 className="text-sm font-semibold">{t.bsTitle}</h4>
        <Badge value={reliable ? t.bsCigtZero : t.bsCiIncludesZero} good={reliable} />
      </div>
      <p className="text-xs text-muted-foreground">
        {t.bsDesc.replace("{n}", bs.n_bootstrap.toLocaleString()).replace("{conf}", (bs.confidence * 100).toFixed(0))}
      </p>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-1 rounded-xl border border-border/60 bg-muted/20 p-3">
        <Stat label={t.bsObservedSharpe} value={bs.observed_sharpe.toFixed(2)} />
        <Stat label={`${(bs.confidence * 100).toFixed(0)}% CI`} value={`[${bs.ci_lower.toFixed(2)}, ${bs.ci_upper.toFixed(2)}]`} />
        <Stat label={t.bsMedianSharpe} value={bs.median_sharpe.toFixed(2)} />
        <Stat label={t.bsProbPositive} value={pctFmt(bs.prob_positive)} />
      </div>
      {/* CI bar */}
      <div className="space-y-1">
        <div className="flex justify-between text-[10px] text-muted-foreground font-mono">
...
function WalkForwardSection({ wf }: { wf: NonNullable<ValidationData["walk_forward"]> }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <h4 className="text-sm font-semibold">{t.wfTitle}</h4>
        <Badge value={`${wf.profitable_windows}/${wf.n_windows} ${t.wfProfitable}`} good={consistent ? true : wf.consistency_rate >= 0.5 ? null : false} />
      </div>
      <p className="text-xs text-muted-foreground">
        {t.wfDesc.replace("{n}", String(wf.n_windows))}
      </p>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-1 rounded-xl border border-border/60 bg-muted/20 p-3">
        <Stat label={t.wfConsistency} value={pctFmt(wf.consistency_rate)} />
        <Stat label={t.wfAvgReturn} value={pctFmt(wf.return_mean)} sub={`std ${pctFmt(wf.return_std)}`} />
        <Stat label={t.wfAvgSharpe} value={wf.sharpe_mean.toFixed(2)} sub={`std ${wf.sharpe_std.toFixed(2)}`} />
        <Stat label={t.wfWindows} value={String(wf.n_windows)} />
      </div>
      {/* Per-window table */}
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b text-left text-muted-foreground">
            ...
            <th className="py-1.5 pr-3">{t.wfColPeriod}</th>
            <th className="py-1.5 pr-3 text-right">{t.wfColReturn}</th>
            <th className="py-1.5 pr-3 text-right">{t.wfColSharpe}</th>
            <th className="py-1.5 pr-3 text-right">{t.wfColMaxDD}</th>
            <th className="py-1.5 pr-3 text-right">{t.wfColTrades}</th>
            <th className="py-1.5 text-right">{t.wfColWinRate}</th>
          </tr>
        </thead>
        <tbody>
          {wf.windows.map((w) => (
...
export function ValidationPanel({ data }: Props) {
  ...
  if (!hasMC && !hasBS && !hasWF) {
    return <p className="p-8 text-sm text-muted-foreground">{t.noValidationData}</p>;
  }
